...
# Convert to lists from JSON strings. (Note: must be STRINGS.)
...
# Create a data frame for each rater. 
dat1 <- as.data.frame(rev1c)
dat2 <- as.data.frame(rev2c)

# Merge data frames into one, by subject (e.g., "Rambo"). 
total <- merge(dat1, dat2, by="row.names")
total$rev1 <- unlist(total$rev1)
total$rev2 <- unlist(total$rev2)

# Leave off the first column which contains subject names ("Rambo").  
data <- total[,c(2,3)]

# Invoke the function from irr library. 
